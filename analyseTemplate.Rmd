...
distrib = by(access_log, access_log$category, function(x) list(nrow(x), round(mean(x$response.time_millis)), round(quantile(x$response.time_millis, c(.25, .50,  .75, .95, 1)))))
...
distrib = by(accesslog_with_size, accesslog_with_size$category, function(x) list(round(sum(x$response.size)/1024/1024,1), round(mean(x$response.time_millis)), round(quantile(x$response.time_millis, c(.25, .50,  .75, .95, 1)))))
...
distrib = by(accesslog_with_size, accesslog_with_size$top_ip, function(x) list(round(sum(x$response.size)/1024/1024,1), round(mean(x$response.time_millis)), round(quantile(x$response.time_millis, c(.25, .50,  .75, .95, 1)))))
displ = analyseDistribution(accesslog_with_size$response.size, distrib, "Total downloaded (MB)")
pander(displ, style = 'rmarkdown', split.table = 1000)
```
