...
access_log$status_int = as.integer(as.character(access_log$status))

g = ggplot(access_log[server.errors,], aes(x=status_int)) + geom_histogram(colour="black", fill="red") +
  labs(title = "HTTP errors distribution")
print(g)
access_log$status_int <- NULL
```
...
distrib = by(accesslog_with_size, accesslog_with_size$category, function(x) list(round(sum(x$response.size)/1024/1024,1), round(mean(x$response.time_millis)), round(quantile(x$response.time_millis, c(0, .25, .50,  .75, .95, 1)))))
...
distrib = by(accesslog_with_size, accesslog_with_size$top_ip, function(x) list(round(sum(x$response.size)/1024/1024,1), round(mean(x$response.time_millis)), round(quantile(x$response.time_millis, c(0, .25, .50,  .75, .95, 1)))))
displ = analyseDistribution(accesslog_with_size$response.size, distrib, "Total downloaded (MB)")
pander(displ, style = 'rmarkdown', split.table = 1000)
```
