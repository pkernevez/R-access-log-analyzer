...
if (max(access_log$response.time_millis) == 0) {
  cat('<span style="color:red">The accesslog file doesn\'t contain duration data. Don\'t take care of duration reports. You should add "%D" at the end of the accesslog format to enable duration analyse.</span>\n')
}
``` 
...
displ = analyseDistribution(accesslog_with_size$response.size, distrib, "Total bandwidth (MB)")
pander(displ, style = 'rmarkdown', split.table = 1000)
```
...
## Response size distribution per client IP
```{r echo=FALSE, h=B_HEIGHT, w=B_WIDTH}
log("Creating all_responsesize_distribution_by_type")
xmax = quantile(accesslog_with_size$response.size, c(PERCENTILE_FOR_DISTRIBUTION))
xmin = min(accesslog_with_size$response.size)
...
## Top 5 big requests and distribution per category
<!-- Reponse time table -->
```{r echo=FALSE, comment=NA, results='asis', h=B_HEIGHT, w=B_WIDTH}
for (cat in CATEGORY_NAMES) {
cat("### ", cat)
